window.onload=function(){
    ...
    activityOption.onchange = onActivityOptionChamge;
}

// let activity = {
//     adventures: ["Six Flags Great Adventure ", "Lakota Wolf Preserve","Absolute Cycle Experience", "Speed Racing ","Legendary Exotics","Bury The Hatchet Cherry Hill"],
//     museums:["New Jersey State","The Newark Museum of Art","Avaition Hall Of Fame", "Paterson Museum" ],
//     wineTasting: ["Cream Ridge Winery"," Cava Winery & Vineyard, Hamburg", "Spuntino Wine Bar & Italian Tapas, Clifton","Coda Rossa, Franklinville"],
//     casino: ["Resort Casino Hotel", "Wild Wild West Casino","Golden Nugget Atlantic City Hotel","Borgata","hard Rock Hotel $ Casino"]
// }
...
function activityDropdown(){
    ...
    function onActivityOptionChamge( ){

    const activityOption=document.getElementById("listCategories")
    let activityOptionValue = activityOption.value;
    let activityExpanded = document.getElementById("ActivitiesExpanded").value
    let getNameFunctionValue = getNameById(activities,categories)
    if (activityOptionValue == "Adventures"){
        let theOption = new Option (getNameFunctionValue)
        activityExpanded.appendChild(theOption);
    };
        
    };

    function getNameById(activities, categories){
        let result =[];
        let activitiesLength = activities.length;
        for (let i = 0 ; i < activitiesLength; i++){
            if (activities.categories==categories)
            result= academy[i].name
        }
        return result;
    };